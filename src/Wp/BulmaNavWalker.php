...
class BulmaNavWalker extends \Walker_Nav_Menu
{
	...
	private function get_item_classes ($item, $args)
	{
		...
		else if ($this->isMenuItemParentOfCurrentPage($item)) {
		    $class_names .= ' is-active';
        }
	...
	private function isMenuItemParentOfCurrentPage($item)
    {
        if (isset($this->menuHierarchy[$item->object_id])) {
            global $post;

            foreach ($this->menuHierarchy[$item->object_id] as $parent => $potentialChild) {
                if (is_numeric($potentialChild)) {
                    if ($post->ID == $potentialChild) {
                        return true;
                    }
                }

                if (is_string($potentialChild)) {
                    if ($post->post_type == $potentialChild) {
                        return true;
                    }
                }
            }
